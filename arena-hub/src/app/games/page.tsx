  ...
  useEffect(() => {
    (async () => {
      const { data, error } = await supabase
        .from("games")
        .select("id,slug,title,active,is_active,banner_path,banner_url")
        .order("title");
  ...
  useEffect(() => {
    ...
    (async () => {
      const toSign = rows.filter(
        (r) => !r.banner_url && r.banner_path && !r.banner_path.startsWith("http")
      );
  ...
  return (
    <div className="container mx-auto px-4 py-6 space-y-4">
      ...
      <section className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {filtered.map((g) => {
          const url = g.banner_url
            ? g.banner_url
            : g.banner_path
            ? signed[g.id] || ""
            : "";
          const active = (g.active ?? g.is_active) ?? true;
          ...
          return (
            <Link key={g.id} href={href} className="card overflow-hidden hover:bg-white/5">
              <div className="aspect-[16/9] w-full bg-black/30">
                {url ? (
                  <img src={url} alt={g.title} className="w-full h-full object-cover" loading="lazy" />
                ) : null}
              </div>
              <div className="p-3 flex items-center justify-between">
                <div className="font-bold">{g.title}</div>
                <span className={`chip ${active ? "chip-primary" : ""}`}>
                  {active ? "فعال" : "غیرفعال"}
                </span>
              </div>
            </Link>
